...
import styles from '@/features/planning/SixMonthCheckinModal/SixMonthCheckinModal.module.css';
...
function SixMonthCheckinModal({ isOpen, onClose, onOpenTimeline, allAxesData }) {
    ...
    useEffect(() => {
        if (isOpen) {
            const first = allAxesData.find(a => a.yearlyGoal);
            if (first) setSelectedAxisId(first.id);
        } else {
            setResponses({});
            setError('');
            setSuccessMessage('');
