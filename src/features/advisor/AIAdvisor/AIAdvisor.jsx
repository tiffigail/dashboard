...
import styles from '@/features/advisor/AIAdvisor/AIAdvisor.module.css';
...
Raw: ${rawPreview}${truncated}`;
  ...
  const streamResponse = async (msgHistory, promptOverride, allowCorpusQuery = true) => {
    ...
    try {
      ...
      const response = await fetch(import.meta.env.VITE_ADVISOR_FUNCTION_URL, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(body.error || `Function error ${response.status}`);
      }

      const { text: fullResponse } = await response.json();
