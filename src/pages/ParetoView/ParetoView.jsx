...
import styles from '@/pages/ParetoView/ParetoView.module.css';
import { db } from '@/firebaseConfig';
import { collection, onSnapshot } from "firebase/firestore";

import AxisOverview from '@/components/shared/AxisOverview/AxisOverview';
import FinancialOverview from '@/features/financial/FinancialOverview/FinancialOverview';
...
function ParetoView() {
    ...
    useEffect(() => {
        ...
        const buildAndSet = () => {
            if (!latestAxes || !latestGoals) return;
            const goalsByAxis = new Map();
            latestGoals.forEach(goal => {
                if (!goalsByAxis.has(goal.axisId)) goalsByAxis.set(goal.axisId, {});
                goalsByAxis.get(goal.axisId)[goal.type] = goal;
            });
            const combinedData = latestAxes.map(axis => ({
                ...axis, goals: goalsByAxis.get(axis.id) || {}
            }));
            const sortedAxes = axisDisplayOrder.map(id => combinedData.find(a => a.id === id)).filter(Boolean);
            setAllAxesData(sortedAxes);
            setIsLoading(false);
        };

        const unsubAxes = onSnapshot(
            collection(db, "new_axes"),
            (snap) => { latestAxes = snap.docs.map(doc => ({ id: doc.id, ...doc.data() })); buildAndSet(); },
            (err) => { console.error("Error loading axes:", err); setError("Failed to load Pareto axis data."); setIsLoading(false); }
        );

        const unsubGoals = onSnapshot(
            collection(db, "new_goals"),
            (snap) => { latestGoals = snap.docs.map(doc => ({ id: doc.id, ...doc.data() })); buildAndSet(); },
            (err) => { console.error("Error loading goals:", err); setError("Failed to load Pareto axis data."); setIsLoading(false); }
        );

        return () => { unsubAxes(); unsubGoals(); };
    }, []);
    ...
    return (
        <div className={styles.paretoViewContainer}>
            {/* Main content grid: 2 columns, 3 rows */}
            <div className={styles.contentGrid}>

                {/* Row 1: Pareto Header (Left) and The Premise (Right) */}
                <header className={`${styles.infographicHeaderLeft} ${styles.gridRow1Left}`}>
                    <div className={styles.mainTitleRow}> {/* New wrapper for title and 80/20 */}
                ...
                {/* Row 2: Methodology (Left) and 5-Year Roadmap (Right) */}
    ...
    <div className={styles.timeline}>
        <div className={styles.timelineItem}>
            <div className={styles.timelineDot}></div> {/* Dot element */}
            <div className={styles.timelineTextContent}> {/* Text content wrapper */}
        ...
        <div className={styles.timelineItem}>
            ...
            <div className={styles.timelineTextContent}> {/* Text content wrapper */}
                ...
                {/* Row 3: Key Tools (Left) and Seven Axes (Right) */}
                ...
                <section id="axes" className={`${styles.gridSection} ${styles.axesSection} ${styles.gridRow3Right}`}>
                    <div className={styles.gridSectionHeaderCenter}>
                        <h2 className={styles.sectionTitle}>The Seven Axes of Focus</h2>
                        <p className={styles.sectionSubtext}>Life is divided into seven key areas for observation. By identifying the most effective 20% of actions in each, 100% of total effort becomes highly effective.</p>
                    </div>
                    {isLoading ? <p className={styles.loadingText}>Loading axes...</p> : error ? <p className={styles.errorText}>{error}</p> :
                        <div>
                            <h3 className={styles.groupTitle}>The 5 Main Axes</h3>
                            <div className={styles.axisGrid}>{mainAxes.map(axis => <AxisCard key={axis.id} axis={axis} />)}</div>
                            <h3 className={styles.groupTitle}>The 2 Support Axes</h3>
            ...
            <AxisOverview isOpen={selectedAxis !== null} onClose={handleCloseAxisModal} axisData={selectedAxis} onDataUpdated={() => {}} axisColor={(axisColors[selectedAxis?.id] || axisColors.default).medium} />
        </div>
    );
}
