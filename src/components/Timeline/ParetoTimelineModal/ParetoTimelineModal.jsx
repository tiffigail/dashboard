...
function ParetoTimelineModal({ isOpen, onClose }) {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            setAllAxes(axesList);
            // LOG for Pareto:
            console.log("ParetoTimelineModal: Fetched Axes List", axesList);
            ...
            // LOG for Pareto:
            console.log("ParetoTimelineModal: Fetched Goals (yearly/stretch)", goalsList);

            const dataByAxis = axesList.map(axis => ({ ...axis, items: goalsList.filter(goal => goal.axisId === axis.id) }));
            setTimelineData(dataByAxis);
            // LOG for Pareto:
            console.log("ParetoTimelineModal: dataByAxis (before sorting/filtering for display)", dataByAxis);
    ...
    // LOG for Pareto:
    console.log("ParetoTimelineModal: filteredData (after user filter):", filteredData);
    console.log("ParetoTimelineModal: paretoAxesData (Core - mapped by AXES_ORDER):", paretoAxesData);
    console.log("ParetoTimelineModal: supportAxesData (Support - mapped by SUPPORT_AXES_ORDER):", supportAxesData);
