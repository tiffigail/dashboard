...
function ParetoView() {
    ...
    const fetchRefactoredParetoData = useCallback(async () => {
        ...
        setError(null);
        try {
            const [axesSnapshot, goalsSnapshot] = await Promise.all([ getDocs(collection(db, "new_axes")), getDocs(collection(db, "new_goals")) ]);
            ...
            axesSnapshot.forEach(doc => {
                const axis = { id: doc.id, ...doc.data() };
                combinedData.push({ ...axis, goals: goalsByAxis.get(axis.id) || {} });
            });
