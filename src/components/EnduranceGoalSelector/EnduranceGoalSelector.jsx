...
import styles from './EnduranceGoalSelector.module.css';
...
function EnduranceGoalSelector({ userId, onSave, onBack }) {
  ...
  const handleSelect = async (journey) => {
    ...
    setIsCreating(true);
    try {
      const result = await enduranceGoalsService.createEnduranceGoal(userId, journey.id);
      console.log("Endurance goal created:", result);
      onSave();
    } catch (error) {
